...
**What this is not.** Not the PRD, and not a metamodel artefact — `Handoff` is working material, deliberately outside the artefact graph (no `handoff` type exists in the registry, by design).

**Where things live.** This folder holds PRD-0001's preparation material and is named for the PRD it serves. The PRD artefact itself must land **beside** this folder, at its registry path — `docs/product-specs/prds/prd-0001-trustworthy-artefact-persistence.md` — never inside it: [ADR-0002](../../../architecture/decisions/adr-0002-artefact-file-binding.md) layout enforcement ([C2.3.F03](../../07a-fbs.md#c23--file-binding-management)) rejects any other path for a `one-per-artefact` type, and the skill's ID discovery (`scripts/get-next-id.py`, non-recursive `glob("*.md")`) only sees PRDs directly in `prds/`. The slug is fixed by this folder's name so the pair reads as one unit; it mirrors the epic it specifies (E-01 · Trustworthy Artefact Persistence), since `SPECIFIES epic → prd` is 1:1.
...
5. **Path:** `docs/product-specs/prds/prd-0001-trustworthy-artefact-persistence.md` — directly in `prds/`, as a sibling of this handoff's folder, not inside it.
...
- [ ] PRD at `docs/product-specs/prds/prd-0001-trustworthy-artefact-persistence.md` with conformant OKF frontmatter; §0 traces E-01, P-01, the C-N.M capabilities, and UC-01–UC-05.
