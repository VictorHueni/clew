...
None at present. *(Functionality-level status is tracked via the ⬜/🔄/✅ column in each capability table above, not as OI rows. Capability-level structural questions live in [`docs/business/03a-capability-map.md` §Open Items](../business/03a-capability-map.md#open-items); delivery sequencing lives in [`docs/plans/delivery-roadmap.md`](../plans/delivery-roadmap.md) — every active backlog row is assigned to an E-NN epic there.)*
...
| Date | Change | Author |
|---|---|---|
| 2026-07-24 | Delivery roadmap minted at [`docs/plans/delivery-roadmap.md`](../plans/delivery-roadmap.md) (registry-canonical path; the stale `docs/product-specs/08a-delivery-roadmap.md` pointer in §Open Items corrected). Every active backlog row now belongs to exactly one epic: E-01 (25 rows, write path), E-02 (8, read side), E-03 (4, C5.2.F01–F04), E-04 (C1.2.F04, gated), E-05 (C5.2.F05 + _TODO_, gated), E-06 (6, C5.1/C5.3/C5.5 remainder); C4.3/C4.4/C3.2.F04 stay outside every epic per §Scope discipline. No status changes (⬜→🔄 promotion happens per-row when a PRD commits the work). | Victor Hueni |
