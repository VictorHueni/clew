...
| OI-0080 | execution-item | Sync the kit registry projection with [ADR-0017](../architecture/decisions/adr-0017-multi-artefact-file-contract.md)'s new type rows — `prd` path confirmation (`docs/product-specs/prds/prd-{nnnn}-{slug}.md`) + `user_story` (`PRD-NNNN.US-NN`, inherits-from-parent, parent `prd`) — and the umbrella-frontmatter rule (D3), clew-side rows authored first per the [ADR-0008](../architecture/decisions/adr-0008-clew-canonical-source-of-truth-for-metamodel.md) chain; coordinate with OI-0078's `package`/`requires` field sync so the kit projection is touched in one pass. | #the-catalogue | The catalogue | Kit-side PR updating the metamodel skill's `artefact-types-registry.yaml` (prd/user_story rows) and `artefact-frontmatter.md` (umbrella rule for mixed files); bundle with the OI-0078 registry-field sync per the ADR-0008 ownership chain. | medium | open | Victor Hueni | 2026-09-30 | _TBD_ |
