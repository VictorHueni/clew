...
- Language fluency: the builder is **Java-primary**, but **already proficient with Typer and FastAPI** (hence Pydantic) — so the net-new surface is effectively just SQLAlchemy Core + Alembic, not the language. Re-examined 2026-06-11 (see Decision Outcome §Java reconsidered): the gap is small, temporary, and AI-agent-mitigated, so it does not select the language — the permanent ecosystem drivers above do
...
- Would supersede ADR-0012 (Alembic → Flyway/Liquibase) with no offsetting structural gain
...
- [ADR-0012 Schema migration framework (Alembic + SQLAlchemy)](adr-0012-schema-migration-framework.md) — **amends** this ADR: SQLAlchemy Core + Alembic are added for schema definition + migrations, softening the "zero non-stdlib DB dependency" consequence (the runtime CRUD path may remain stdlib `sqlite3`). Adoption is itself **deferred to v1's hand-rolled path by [ADR-0013](adr-0013-minimal-model-not-repo-native-ea.md)** (minimal-model rationale).
