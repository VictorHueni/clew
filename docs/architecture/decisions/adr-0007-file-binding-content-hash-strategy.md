...
| 2026-07-24 | **Amended by [ADR-0013](adr-0013-minimal-model-not-repo-native-ea.md)** (banner added): C4.3 audit-trail capability cut, audit delegated to git; the hash strategy now stands on drift detection (C4.2) alone, decision unchanged. OI-0029 → `closed` (forward pointer verified present in ADR-0002). OI-0032 → `closed` (CLI contract `clew check` four-category output + `clew bind --update [--all]` documented in [cli-clew.md](../interfaces/cli-clew.md) the same day). | Victor Hueni |
