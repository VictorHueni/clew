...
| OI-0032 | doc-gap        | [`docs/architecture/interfaces/cli-clew.md`](../interfaces/cli-clew.md) `clew check` command and `clew bind` subcommand still need to document the four-category output and the `--update [--all]` flag.                          | #decision-outcome              | Decision Outcome — Phase 2                                | Update the CLI contract `clew check` section to list the four categories with example output; document `clew bind --update <id>` and `clew bind --update --all` flag semantics. Reference this ADR.          | high     | open   | Victor Hueni | 2026-06-30        | _TBD_       |
