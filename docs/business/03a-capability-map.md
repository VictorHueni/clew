...
- **Soft-link gaps.** Value streams exist ([`04a-value-streams.md`](04a-value-streams.md)) with VS-1, VS-2, and VS-3 fully filled. 16 of 19 L1 capabilities carry a `Realised in streams:` soft-link: C1.1, C1.2, C1.3, C2.1, C2.2, C2.3, C2.4, C3.1, C3.2, C3.3, C4.1, C4.2, C4.3, C5.1, C5.2, C5.3. VS-3 added no first-time backlinks (all consumed capabilities were already wired from VS-1 / VS-2; VS-3 is value-flow reuse of existing substrate). The remaining 3 L1 capabilities (C4.4 Schema migration, C5.4 Cross-methodology referencing, C5.5 Methodology canon coverage assessment) await VS-4 (Share) stage decomposition, though C4.4 and C5.5 may legitimately never land in a user-facing stream (they are clew-internal maintenance capabilities; revisit after VS-4 fill). Processes, FBS, and domain model still do not exist; their soft-links remain pending.
