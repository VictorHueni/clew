...
description: clew's strategic what-layer — a stable L0+L1 hierarchy of capabilities independent of the technology, organisation, or processes that realise them.
tags: [business, capability-map]
timestamp: 2026-05-24T21:27:20Z
status: draft
owner: Victor Hueni
...
| OI-0079 | doc-gap        | Cartography export (`clew export likec4`) is the fourth read-side surface per [ADR-0015](../architecture/decisions/adr-0015-opt-in-layer-packages.md) (D7), joining `context`/`trace`/`impact` — currently absent from both this map's C3 decomposition and the FBS (`07a-fbs.md`).  | #c3--querying-and-traceability | C3 · Querying and Traceability  | Add the cartography-export functionality under C3 (new C3.x L1 or a functionality under C3.2) and mirror it in `07a-fbs.md`; preserve ADR-0015's ArchiMate-as-export-only boundary in the definition wording. | medium   | open        | Victor Hueni | 2026-09-30        | _TBD_       |

## Changelog

| Date | Change | Evidence | Cascading effects |
|---|---|---|---|
| 2026-07-24 | OI-0079 filed: cartography export (`clew export likec4`) — the fourth read-side surface decided in ADR-0015 (D7) — is absent from this map's C3 decomposition and from the FBS. Open Items row only; capability content unchanged pending the C3.x placement decision. | [ADR-0015](../architecture/decisions/adr-0015-opt-in-layer-packages.md) §Decision Outcome (D7). | Central ledger row OI-0079 filed in the same pass; the FBS gap is tracked by the same item. |
| 2026-07-24 | Canonical slug backfill (audit Check 19). Every L0 (`## CN ·`) and L1 (`### CN.M ·`) heading gains its mandatory `slug:` code-line per the registry slug contract — 22 slugs minted, derived by slugify(heading name) shortened to the ≤20-char recommendation. Identical C-IDs in the FBS received the byte-identical slugs in the same pass. | [Kit registry §Canonical slugs](../../../homemade-claude-kit/rules/artefact-types-registry.md); stack audit 2026-07-24 §19. | FBS `07a-fbs.md` carries the same 22 slugs + the `clew` product slug; flat namespace now feeds the commit-scope vocabulary. |
