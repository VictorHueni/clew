...
**Upstream gaps:** processes (`proc-NN-{slug}`), FBS functionalities, and quantitative models are not yet defined; soft-link slots referencing them remain `_TODO_`. Value streams are defined in [`04a-value-streams.md`](04a-value-streams.md): VS-1 (Compose), VS-2 (Navigate), and VS-3 (Refactor) are fully filled; VS-4 (Share) is catalogued but not yet stage-decomposed. §3 UVP Concrete wins 1, 2, and 3 are now all wired to filled streams; the markdown-and-git readability bullet still queues on VS-4. §5 Channels soft-links unchanged (no VS-3 dependency). Capabilities are defined in [`03a-capability-map.md`](03a-capability-map.md); §4 and §9 soft-links reference them directly.
...
- **Soft-link gaps:** FBS-_TODO_, Quantitative model _TODO_, Cost model _TODO_, VPC-_TODO_. These wire up as the metamodel fills out. (Capabilities wired 2026-05-24, see [03a-capability-map.md](03a-capability-map.md). Value streams wired 2026-05-25, see [04a-value-streams.md](04a-value-streams.md): VS-1, VS-2, and VS-3 filled; VS-4 catalogued. §3 UVP Concrete wins 1, 2, and 3 are all wired to filled streams; markdown-and-git readability bullet queues on VS-4. A separate "Adopt clew" acquisition value stream would be needed to fully wire §5 Channels Discovery/Support phases; not modelled today because the prospective-P-01 persona does not yet exist.)
