...
These emerged but cannot be answered by N = 1:

1. **Persona generalisation.** Does the "any builder" framing generalise to QA / DevOps / SRE personae who reach for clew, or are they too different from P-01 to share a substrate? Wave 2 should include at least one non-product role.
2. **Adoption-commitment gap.** A P-01 stranger has to bet on a 3-month commitment beyond initial demo-trigger. What evaluation criteria do they apply that the founder cannot self-assess?
3. **Methodology canon depth.** Which methodologies are missing from the current encoding given the broadened lifecycle scope? Candidates: DDD (Evans / Vernon / Wlaschin) for domain modelling, ATDD / BDD (North / Beck) for testing, SRE discipline (Beyer et al.) for ops / runbook.
4. **Token-economics quantification.** How much agent-cost saving does local-first actually deliver vs. MCP-mediated tooling, at what scale? Speculative until measured.
5. **Schema migration cost.** What's the v0.1 → v0.2 schema migration story? Exit-ramp risk is unknowable to a prospective adopter without it.

These should inform wave-2 interview scripts and may justify expanding KR-03.2's target from "3 to 5" to "5 to 8" if external interviewees are reachable across roles.
