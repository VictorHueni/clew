...
- Ask the architecture cross-cutting questions: "what does this epic serve?" / "which artefacts reference this persona?" / "where is this domain area used?" *(Realised by [VS-2 Navigate Architecture](04a-value-streams.md#catalogue), catalogued; stages not yet decomposed.)*
- Refactor with confidence: rename, re-tier, or retire an artefact and see the full downstream impact before committing. *(Realised by [VS-3 Refactor Architecture](04a-value-streams.md#catalogue), catalogued; stages not yet decomposed. Write-time integrity already partially realised at [VS-1.4 Persist](04a-value-streams.md#vs-14--persist-with-stable-id).)*
- Share the architecture in a form colleagues can review alongside the work, including those who do not use the tool directly. *(Realised by [VS-4 Share Architecture](04a-value-streams.md#catalogue), catalogued; stages not yet decomposed.)*
