...
| Field | Value |
|---|---|
| **Participating stakeholders** | [P-01 Ava](01a-personas.md#p-01--ava-the-agent-first-product-engineer) (commits). The agent executes the change-set as an ordered sequence of validated writes via the clew CLI. |
| **Entrance criteria** | Impact previewed and accepted (VS-3.2 done). |
| **Exit criteria** | Change is applied as a sequence of per-write transactions, each validated at write time; **every intermediate state is referentially legal** — never dangling, never type-invalid — though the sequence is semantically incomplete until it finishes. · New state passes schema validation. · Each step passes reference integrity validation against the store as it then stands; a step that would strand a reference is refused, and a retire is refused while references remain. · File bindings are updated for every rewritten section (path + section anchor + new content hash). · The change-set's who/when/before-after history is recoverable from git on the snapshot directory ([ADR-0013](../architecture/decisions/adr-0013-minimal-model-not-repo-native-ea.md) delegates the audit trail to git). · Deterministic snapshots are regenerated for every affected artefact. |
| **Value items produced** | Applied change-set · regenerated snapshots · preserved referential integrity across the substrate at every step |
| **Enabling capabilities** | [C4.1 Write-time reference validation](03a-capability-map.md#c41--write-time-reference-validation) (Differentiator role at this stage; enforces integrity on every step of the change-set) · [C2.1 Stable identifier generation](03a-capability-map.md#c21--stable-identifier-generation) (IDs preserved through rename; never recycled on retire) · [C2.2 Schema enforcement](03a-capability-map.md#c22--schema-enforcement) · [C2.3 File binding management](03a-capability-map.md#c23--file-binding-management) (bindings rewritten for every section touched) · [C2.4 Deterministic structural export](03a-capability-map.md#c24--deterministic-structural-export) (snapshots regenerated) |
| **Operationalised by processes** | _TODO_ |
| **Pain point index** | **High**. Wave-1 [F2 finding](../discovery/interviews/research-synthesis-2026-05-24-P-01-validation.md): without integrity-validated execution, partial refactors leave the substrate inconsistent (some files updated, some not; some IDs preserved, some lost). C4.1's write-time validation must hold on every step against the store as it then stands, so that an interrupted refactor leaves a *legal but incomplete* state rather than a broken one. *(Tested · N=1.)* |

> **Atomicity scope (decided 2026-07-26).** v1 guarantees **per-write transactions with referentially-legal intermediate states**, not all-or-nothing change-set atomicity. An `apply <changeset>` surface was considered and rejected for v1: it would add rows to an epic already at its 25-row sizing ceiling, and the per-step guarantee already delivers the property that matters — no intermediate state is ever *broken*. Decided in [PRD-0001 §8-D1](../product-specs/prds/prd-0001-trustworthy-artefact-persistence.md#8--open-questions), closing [OI-0084](../../project-control/open-items/open-items.md); the behavioural contract is [UC-02's minimal guarantee](../product-specs/use-cases/uc-02-refactor-artefact-with-foreseen-impact.md#guarantees), which already encoded this baseline.
...
| **Entrance criteria** | Change-set sequence executed, every step validated (VS-3.3 done). |
...
|---|---|---|---|
| 2026-07-26 | **VS-3.3 atomicity wording softened to per-step legality (OI-0084 decided).** The exit criterion "applied atomically across all affected artefacts (all-or-nothing; no partial writes)" was aspirational: the CLI contract guarantees per-invocation transactions only. Replaced with the guarantee v1 actually makes — a sequence of per-write transactions, each validated at write time, **every intermediate state referentially legal** though semantically incomplete until the sequence finishes. §Participating stakeholders, §Value items produced, §Enabling capabilities, and §Pain point index reworded to match; a new §Atomicity scope note records the rejected `apply <changeset>` alternative and why. The stale "Audit trail records the change-set as a single linked event" exit criterion and the C4.3 entry in §Enabling capabilities were removed in the same pass — [ADR-0013](../architecture/decisions/adr-0013-minimal-model-not-repo-native-ea.md) delegates the audit trail to git, so both had been dangling since 2026-07-07. VS-3.4 entrance criteria and pain-index wording follow. | [PRD-0001 §8-D1](../product-specs/prds/prd-0001-trustworthy-artefact-persistence.md#8--open-questions) · [OI-0084](../../project-control/open-items/open-items.md) · [UC-02 §Guarantees](../product-specs/use-cases/uc-02-refactor-artefact-with-foreseen-impact.md#guarantees) | Closes OI-0084. [UC-02](../product-specs/use-cases/uc-02-refactor-artefact-with-foreseen-impact.md) already encoded this baseline in its minimal guarantee — its ext. 7a and §Related Information "open decision" notes updated to decided in the same pass. CLI contract §5 Atomicity unchanged (it already stated per-invocation transactions); a scope note added there for symmetry. No pain-index or confidence changes. |
