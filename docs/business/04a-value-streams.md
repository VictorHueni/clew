...
- **Triggering stakeholder:** [P-01 Ava](01a-personas.md#p-01--ava-the-agent-first-product-engineer) (initiates the sharing). Consumer is [P-02 product partner](01a-personas.md) or other non-clew colleague (downstream value recipient; persona is Tier-2 / Backlog).
- **Value proposition:** A markdown-and-git artefact set any colleague (P-02 product partner or other) can review without authoring tools.
...
- **Pain-index honesty audit.** Wave-1 evidence backs VS-1.2/1.3/1.4, VS-2.2/2.3/2.4, and VS-3.2/3.3/3.4 (N=1, founder-as-instance). Low/Assumed: VS-1.1, VS-2.1, VS-3.1, and the entirety of VS-4 (no consumer yet to validate sharing friction). VS-4 stands out as the only non-Critical filled stream; this is honest, not under-claiming.
- **VS-4 dogfood pass (2026-05-25).** Walked all 4 stages end-to-end as part of finalising the value-stream layer (see commit `87511cb`: README §Where to start reading). Findings:
  - VS-4.1 and VS-4.3 passed trivially (working tree clean; repo public on GitHub). Both Low ratings honest for current cadence and git workflow respectively; both could shift if cadence drops (drift between modelling and sharing) or if clew later ships its own publishing target.
  - VS-4.2 had nothing to materialise because canonical views (C3.2) don't exist yet. When C3.2 implementation lands, VS-4.2 will need explicit tooling decisions: which views to materialise, where to put them, how to keep them fresh. Stage block is currently silent on these. **Revisit on C3.2 implementation landing.**
  - VS-4.4 was the actual deliverable. The pre-existing README was a product pitch and did not serve as a navigation map for the [`docs/business/`](docs/business/) artefacts; the §Where to start reading section closed the exit-criteria gap in ~30 minutes of authoring. Magnitude calibration check confirms **Medium** rating is honest at this artefact-set size; would scale higher if the artefact count grew.
  - Trigger to re-validate VS-4 pain ratings remains **UNFIRED**: the author walking the workflow alone is not "a second user reads the published snapshot." Re-validation still depends on the trigger event below.
- **VS-4 pain-validation trigger.** VS-4's whole pain profile is hypothetical (Assumed across all 4 stages). **Trigger to re-validate:** when a second user first reads the published snapshot, including but not limited to: a GitHub stargazer opening a discussion or issue, a colleague pinged for review, a prospective adopter exploring the docs. At that point, re-rate VS-4.1/4.2/4.3/4.4 from observed friction; the ranking may shift materially (especially VS-4.4 Orientation, which is most likely to surface as Critical the first time a real reader cannot find their entry point). Until the trigger fires, VS-4 pain ratings remain Assumed and should not drive engineering priority over VS-1/2/3.
- **Confidence-cliff watchpoint.** All pain ratings rest on N=1 (founder-as-instance). They are currently *descriptive* (documenting lived experience). They become *prescriptive* the moment a rating is cited to prioritise one engineering item over another (e.g., "build C4.1 before C3.3 because VS-3.2 is Critical"). Wave-2 (see [OBJ-03 KR-03.2](04b-objectives.md#obj-03--validate-the-core-hypotheses-before-scaling)) must happen **before** that crossover. **Trigger:** when a planning conversation cites a pain rating as the deciding factor for sequencing engineering work, pause and run wave-2 before committing the priority.
