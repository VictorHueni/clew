...
- [C4.3 Audit trail](../business/03a-capability-map.md#c43--audit-trail) — *delegated to git (VCS) for v1 per [ADR-0013](../architecture/decisions/adr-0013-minimal-model-not-repo-native-ea.md); see [capability map §Scope discipline](../business/03a-capability-map.md#scope-discipline-adr-0013)* — replayable record of every write; git on `snapshot/` already provides who/when/before-after, durably
...
- [*Skill* · BC-01.GT-15](02c-glossary.md#skill--bc-01gt-15) *(homonym warning)* — in BC-01 not a domain concept; appears only as the kit-side package carrying the *semantic* authoring methodology. Per [ADR-0008](../architecture/decisions/adr-0008-clew-canonical-source-of-truth-for-metamodel.md) artefact-type schemas are clew-owned — the kit's structural registry regenerates from clew ([02b-context-map.md §the ADR-0008 flip](02b-context-map.md#bc-01--customer-supplier--homemade-claude-kit-structural-registry-the-adr-0008-flip)). *Inside the kit's own scope, "skill" is the central abstraction; do not confuse the two usages.*
...
- **Publishes (events):** domain events `ArtefactRegistered`, `ArtefactImported`, `ArtefactLinked`, `FileBindingRecorded`, `SnapshotExported`, `SnapshotRestored` — in-process at v1, with no persisted event log (audit is delegated to git on the committed `snapshot/` per [ADR-0013](../architecture/decisions/adr-0013-minimal-model-not-repo-native-ea.md)); v2 (MCP) may surface them on a wire protocol.
...
| Date | Change | Author |
|---|---|---|
| 2026-07-24 | Decided-ADR cascade. C4.3 (Audit trail) in §Capabilities owned annotated *delegated to git (VCS) for v1* per [ADR-0013](../architecture/decisions/adr-0013-minimal-model-not-repo-native-ea.md), mirroring [capability map §Scope discipline](../business/03a-capability-map.md#scope-discipline-adr-0013); §Integration interfaces events line no longer claims events are "recorded in the audit trail" (in-process at v1; audit delegated to git). §Ubiquitous language scope Skill bullet aligned with [ADR-0008](../architecture/decisions/adr-0008-clew-canonical-source-of-truth-for-metamodel.md) (artefact-type schemas clew-owned; kit registry a generated projection), matching the already-updated §Integration interfaces Customer-Supplier / Conformist lines. | Victor Hueni |
